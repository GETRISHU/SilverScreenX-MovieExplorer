    ...
    async function showMovieDetail(movie) {
      ...
      if (result.videos && result.videos.results.length) {
        ...
        if (yt && yt.key) {
          const youtubeWatchUrl = `https://www.youtube.com/watch?v=${yt.key}`;
          trailer = `
            <div class="mb-4">
              <div class="text-sm text-gray-400 mb-2">Trailer embed may not play in all browsers. Use the link below if it does not load.</div>
              <a
                href="${youtubeWatchUrl}"
                target="_blank"
                rel="noopener noreferrer"
                class="inline-flex items-center gap-2 rounded-full bg-[#BB2D3B] px-4 py-2 text-sm font-semibold text-black hover:bg-opacity-90"
              >
      ...
      const homepageLink = result.homepage
        ? `<a href="${result.homepage}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 text-[#BB2D3B] hover:text-white text-sm font-semibold">
            ...
            <div class="flex flex-wrap gap-2 mb-3 items-center">${
              (movie.genre_ids || result.genres || [])
                .map((g) => {
                  ...
                  return gn ? `<span class="px-3 py-1 bg-[#BB2D3B] rounded-full text-sm">${gn}</span>` : "";
                })
                .join("")
            }${homepageLink ? `<span class="ml-2">${homepageLink}</span>` : ""}</div>
